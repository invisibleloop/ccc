...
/**
 * Validates that the commit header doesn't exceed 72 characters
 * @param {Object} data - The commit data
 * @returns {Object} Object with { valid: boolean, length: number, limit: number }
 */
export function validateHeaderLength(data) {
  const HEADER_MAX_LENGTH = 72;

  // Calculate header length: type(scope)!: description
  let headerLength = data.type.length;
  if (data.scope) {
    headerLength += data.scope.length + 2; // +2 for ()
  }
  if (data.isBreaking) {
    headerLength += 1; // +1 for !
  }
  headerLength += 2; // +2 for ": "

  // Ensure description starts with lowercase
  const description = data.description.charAt(0).toLowerCase() + data.description.slice(1);
  headerLength += description.length;

  return {
    valid: headerLength <= HEADER_MAX_LENGTH,
    length: headerLength,
    limit: HEADER_MAX_LENGTH
  };
}
