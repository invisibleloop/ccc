 ...
 * @param {string[]} [options.allowedTypes] - Restrict commit types to this list
...
export async function generateCommitMessage(diff, options = {}) {
  const allowedTypes = options.allowedTypes || ['feat', 'fix', 'docs', 'style', 'refactor', 'perf', 'test', 'build', 'ci', 'chore', 'revert'];
...
1. CRITICAL: Type MUST be one of these exact values: ${allowedTypes.join(', ')} — any other value is invalid
2. Scope is optional (e.g., parser, api, ui) - keep it short, use abbreviations if needed
3. Description should be short, start with lowercase, no period at end
4. CRITICAL: The header (type + scope + description) must NOT exceed 72 characters total
   - Example: "feat(api): add support for array parsing" is 41 chars (good)
   - Keep scope and description concise to stay within this limit
5. Identify if this is a breaking change
